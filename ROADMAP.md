...
| UX-002 | Medium | Open | Settings | Server health check before switching | Add an explicit server-row health check flow: tapping a saved non-current server probes readiness first, shows reachable/auth-required/setup/offline status, and asks for confirmation before switching; include safe diagnostic breadcrumbs for the check result |

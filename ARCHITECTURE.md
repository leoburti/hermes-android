...
4. Android WebView compatibility shims stay scoped to Hermes WebUI. Android disables native WebView long-press action-mode handling, injects a document-start long-press context-menu shim to synthesize `contextmenu` events even when native `touchcancel` fires, and injects a document-start microphone fallback so WebUI voice input uses its MediaRecorder path instead of Web Speech API. The official dashboard is not rendered in an app WebView.
...
- Android suppresses native WebView long-press action-mode handling via `isLongClickable = false` and `setOnLongClickListener { true }`, and injects a scoped long-press context-menu shim at document start. The shim synthesizes `contextmenu` events reliably even when Android's native gesture detector fires `touchcancel` before Hermes WebUI's own long-press timer completes.
